...
# class Img_pach:
#     def __init__(self, file_imgs):
#         self.file_data = file_imgs
...
class Convertor(object):
    ...
    def __init__(self, path, img_path, config=FLAG.CF_INDEXED_4_BIT, dith=True):
...
const lv_img_dsc_t {self.out_name} = {{
  ...
  .data_size = '''
    ...
    def get_bin_file(self, cf=-1, content=None) -> bytes:
        ...
        # file_imgs = img_path()
        print(self.img_path1)
        with open(self.img_path1 + "/" + self.out_name + ".bin", "wb") as f:
